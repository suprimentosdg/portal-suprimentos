...
with st.form(key="include_called"):
    ...
    region2 = st.selectbox("Selecione sua regiona:", regions)
    store2 = st.number_input("Digite sua loja:", max_value=999)
    ...
    obs = st.text_area("Obs.:", placeholder="Em caso de assistência técnica, informe o motivo da solicitação.")
    ...
    if send2:
        ...
        if not name2 or not region2 or not store2 or not printerTypes or not options:
            ...
            st.warning("Por favor, preencha o motivo do chamado.")
